...
class VM {
	...
	private var vtPalette:Array<Array<UInt>>; // NES palette set(调色板集)
	...
	private function run():Void {
		...
		// 	return;
		// }
		var vm_ft:Int = Std.int(Date.now().getTime());//getTimer();
		...
		// 	var x:Int = i % 256;
		// 	bmd.setPixel(x,y,pix);
		// 	//trace(x,y,"...");
			
		// }

		var newPixels:Bytes = Bytes.alloc(bus.ppu.vtIMG.length * 4);
		...
		// for(x in 0...256){
		// 	for(y in 0...240){
		// 		TV?.setPixel(x,y,0xffffffff);
		// 	}
		// }
		
		//trace(bus.ppu.vtIMG.length);
		...
		while (true) {
			...
			if (nextScanline == 0) {
				vm_ft = Std.int(Date.now().getTime()) - vm_ft; //openfl.Lib.getTimer() - vm_ft;
				runTime += vm_ft;
